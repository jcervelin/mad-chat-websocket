...
import io.jcervelin.plugins.MockOpenAIClient
import io.jcervelin.plugins.configureRouting
import io.jcervelin.services.AIClient
import io.jcervelin.services.OpenAIChatMessage
import io.jcervelin.services.OpenAIChatRequest
...
class ApplicationTest {
    ...
    """)
        ) + listOf(OpenAIChatMessage("user", "Testing 123 content"))

        val encodeToJsonElement = DefaultJson.encodeToJsonElement(
            OpenAIChatRequest(
                model = "gpt-3.5-turbo",
                messages = messages,
                temperature = 0.7,
        ...
        println(encodeToJsonElement.toString())

        application {
            configureRouting(chatRoom, MockOpenAIClient(), history, Clock.systemUTC())
        }
